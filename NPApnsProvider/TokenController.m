...
-(void)addClicked:(id)sender
{
    ...
    int count = (int)[tokenContainer count];
    [tokenContainer addToken:token withName:[NSString stringWithFormat:@"%d",count]];
    [_tokenSV.contentView.documentView reloadData];
...
- (IBAction)selectJsonFile:(id)sender {
    NSOpenPanel *openPannel=[NSOpenPanel openPanel];
    openPannel.canChooseDirectories=NO;
    openPannel.canChooseFiles=YES;
    ...
    [openPannel beginSheetModalForWindow:[[NSApplication sharedApplication]keyWindow] completionHandler:^(NSModalResponse result){
        if(result==NSFileHandlingPanelOKButton)
        {
            NSArray *tokenURLs=[openPannel URLs];
            BOOL found=NO;
            if([tokenURLs count] >0){
                NSString *pathOfToken=[tokenURLs[0] path];
                NSData *data=[NSData dataWithContentsOfFile:pathOfToken];

                NSArray* tokens = [NSJSONSerialization JSONObjectWithData:data options:NSJSONReadingMutableLeaves error:nil];
                for(int i=0;i<[tokens count];i++){
                    NSDictionary* dic = tokens[i];
                    NSString* tokenStr = [dic valueForKey:@"deviceToken"];
                    if (tokenStr.length == 64) {
                        NSData *token=[self convertToken:tokenStr];
